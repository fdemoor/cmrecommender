...
def get_random_parameters(iteration):
  if iteration >= len(random_parameters):
    for i in range(len(random_parameters), iteration + 1):
      a = random.randint(1, sys.maxsize)
      b = random.randint(1, sys.maxsize)
      random_parameters.append([a, b])
  return random_parameters[iteration][0], random_parameters[iteration][1]
...
class Sketch:
    def __init__(self, delta, epsilon, k, t):
        ...
        if epsilon <= 0 or epsilon > np.exp(1):
            print(epsilon)
    ...
    def get(self, key):
        ...
        for row, hash_function in enumerate(self.hash_functions):
            column = hash_function(abs(hash(key)))
            # print "KEY-WIDTH-ROW-COLUMN", key, self.w, row, column
    ...
    def __generate_hash_function(self, iteration):
        ...
        """
        a, b = get_random_parameters(iteration)
        # print "a: %d - b: %d" % (a, b)
